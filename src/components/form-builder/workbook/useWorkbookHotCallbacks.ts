...
export const useWorkbookHotCallbacks = ({
  ...
  const afterChange = React.useCallback(
    (changes: any, source: string) => {
      ...
      // Synchronous: update formula/cell metadata before returning so that
      // getWorkbookSnapshot() called in the same tick (e.g. the "Save Changes"
      // button handler) always reads correct, up-to-date cellMeta.
      if (isEligibleChange) {
        onCellChanges?.(
      ...
      setTimeout(() => {
        ...
        // True when the user navigated to a different tab while this timer queued.
        const sheetSwitched = activeSheetIndexRef.current !== changeSheetIndex;

        // yesNoSync: mutate the live HOT DOM — only safe on the same sheet;
        // after a switch the HOT instance belongs to a different sheet.
        if (isEligibleChange && !sheetSwitched && hot) {
